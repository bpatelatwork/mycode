...
#Provide form to remove a student using name and pin
@app.route('/removeastudent/<studentname>')
def remove_student(studentname):
    con= sql.connect("database.db")
    con.execute(f"DELETE FROM students where name = {studentname};")
    con.commit()
    return redirect("/")


#Remove the provided student info from the database
@app.route('/removerec',methods = ['POST'])
def removerec():
    ...
    try:
        ...
        with sql.connect("database.db") as con:
            ...
            # place the info from our form into the sqliteDB
            cur.execute("DELETE FROM students WHERE (name,pin) VALUES (?,?)",(nm,pin) )
            # commit the transaction to our sqliteDB
            con.commit()
        # if we have made it this far, the record was successfully added to the DB
...
if __name__ == '__main__':
    try:
        ...
        con.execute('CREATE TABLE IF NOT EXISTS students (name TEXT PRIMARY KEY, addr TEXT, city TEXT, pin TEXT)')
